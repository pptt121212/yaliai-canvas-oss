import { CANVAS_REFERENCE_IMAGE_MAX_BYTES, CANVAS_REFERENCE_IMAGE_MAX_COUNT } from './workflow.js';
import {
  getDefaultEcommerceSetImageTypes,
  getEcommerceCapability,
  getEcommerceCapabilityConfig,
  getEcommerceImageSetCount,
  getEcommerceModuleTypeOptions,
  normalizeEcommerceSetImageTypes,
} from './ecommerce.js';

const LOCAL_BROWSER_SUPPORTED_NODE_TYPES = new Set([
  'prompt',
  'batchPrompt',
  'reference',
  'localReference',
...
const OUTPUT_FORMAT_MIME = {
  png: 'image/png',
  jpeg: 'image/jpeg',
  webp: 'image/webp',
};

export function isLocalBrowserSupportedNodeType(type) {
  return LOCAL_BROWSER_SUPPORTED_NODE_TYPES.has(String(type || '').trim());
}

export function getLocalBrowserUnsupportedMessage() {
  return '当前节点不支持在本地模式中运行。请切换到登录模式。';
}

export function resolveLocalCanvasNodeSize(data = {}) {
...
export function resolveLocalCanvasNodeOutputFormat(data = {}) {
...
export function collectLocalPromptItems(node, nodes, edges) {
  const incoming = getIncomingNodes(node.id, nodes, edges);
  ...
  return fallback ? [{ prompt: fallback, batchItem: null }] : [];
}

export function collectLocalReferenceSets(node, nodes, edges, resultsByNode) {
  const incoming = getIncomingNodes(node.id, nodes, edges);
  const base = [];
  const groups = [];
  ...
  incoming.forEach((source) => {
    ...
    const images = getNodeResultImages(source, resultsByNode);
    if (!images.length) {
      return;
    }
  ...
  const combinations = groups.reduce((acc, group) => {
    ...
    const next = [];
    acc.forEach((current) => {
      group.forEach((item) => next.push(current.concat(item)));
    });
    return next;
  }, [[]]);

  return combinations.length
    ? combinations.map((combo) => Array.from(new Set(base.concat(combo))))
    : [Array.from(new Set(base))];
}

export function collectLocalAllReferences(node, nodes, edges, resultsByNode) {
  return Array.from(new Set(collectLocalReferenceSets(node, nodes, edges, resultsByNode).flat()));
}

export async function executeLocalImagesRequest({ node, prompt, references, upstreamPreference, signal }) {
  const referenceList = Array.isArray(references)
    ? references.map((item) => String(item || '').trim()).filter(Boolean)
    : [];
  if (referenceList.length > CANVAS_REFERENCE_IMAGE_MAX_COUNT) {
    throw new Error(`单次最多只支持 ${CANVAS_REFERENCE_IMAGE_MAX_COUNT} 张参考图，请减少后再试。`);
  }
  const isEdit = referenceList.length > 0;
  const endpoint = String(
    isEdit
      ? upstreamPreference?.imagesEditsUrl || ''
      : upstreamPreference?.imagesGenerationsUrl || ''
  ).trim();
  if (!endpoint) {
    throw new Error(isEdit
      ? '本地模式缺少图生图完整地址，请先在设置中填写 Images Edits Endpoint。'
      : '本地模式缺少文生图完整地址，请先在设置中填写 Images Generations Endpoint。');
  }

  const data = node?.data || {};
  const outputFormat = resolveLocalCanvasNodeOutputFormat(data);
  const requestBody = {
    model: String(data.model || 'gpt-image-2').trim() || 'gpt-image-2',
    prompt: String(prompt || '').trim(),
    size: resolveLocalCanvasNodeSize(data),
  ...
  const headers = buildAuthHeaders(upstreamPreference);
  let response;
  try {
    if (isEdit) {
      ...
      for (let index = 0; index < referenceList.length; index += 1) {
        const file = await imageSourceToFile(referenceList[index], index, signal);
    ...
    if (message.includes('Failed to fetch') || message.includes('NetworkError')) {
      throw new Error('本地模式请求被浏览器拦截或网络不可达。请确认上游接口已允许当前站点跨域访问，并且地址与密钥正确。');
    }
    throw error;
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(extractErrorMessage(failureText) || `上游接口返回错误：HTTP ${response.status}`);
  }
  ...
  const first = dataItems[0] && typeof dataItems[0] === 'object' ? dataItems[0] : null;
  const directUrl = String(first?.url || payload?.url || '').trim();
  if (directUrl) {
    const localizedUrl = await tryLocalizeRemoteImageUrl(directUrl, signal);
    return {
      taskId: createLocalTaskId(),
      imageUrl: localizedUrl || directUrl,
      raw: payload,
    };
  }

  const b64Json = String(first?.b64_json || payload?.b64_json || '').trim();
  if (b64Json) {
    const mime = OUTPUT_FORMAT_MIME[String(first?.output_format || payload?.output_format || outputFormat).trim().toLowerCase()] || OUTPUT_FORMAT_MIME[outputFormat] || 'image/png';
    return {
      taskId: createLocalTaskId(),
      imageUrl: `data:${mime};base64,${b64Json}`,
      raw: payload,
    };
  }

  const resultValue = String(first?.result || payload?.result || '').trim();
  if (resultValue.startsWith('data:image/') || /^https?:\/\//i.test(resultValue)) {
    const localizedUrl = /^https?:\/\//i.test(resultValue)
      ? await tryLocalizeRemoteImageUrl(resultValue, signal)
      : '';
    return {
      taskId: createLocalTaskId(),
      imageUrl: localizedUrl || resultValue,
      raw: payload,
    };
  }

  throw new Error('上游接口未返回可用图片。请确认该接口兼容标准 Images Endpoint 响应格式。');
}

export async function executeLocalChatCompletion({ upstreamPreference, instruction, images, signal }) {
  const endpoint = String(upstreamPreference?.chatBaseUrl || '').trim();
  const apiKey = String(upstreamPreference?.chatApiKey || '').trim();
  if (!endpoint || !apiKey) {
    throw new Error('本地模式缺少 Chat Completions 地址或密钥，当前节点无法执行。');
  }
  ...
  const content = [
    { type: 'text', text: String(instruction || '').trim() },
  ...
  try {
    ...
    const message = String(error?.message || '');
    if (message.includes('Failed to fetch') || message.includes('NetworkError')) {
      throw new Error('本地模式 Chat 请求被浏览器拦截或网络不可达。请确认 Chat Completions 已允许当前站点跨域访问。');
    }
    throw error;
  }
  ...
  if (!response.ok) {
    throw new Error(extractErrorMessage(text) || `Chat Completions 返回错误：HTTP ${response.status}`);
  }
  return extractChatCompletionText(payload) || String(text || '').trim();
}

const LOCAL_ECOMMERCE_INTERNAL_LABEL_TERMS = [
  '首屏主视觉',
  '商品介绍',
  '核心卖点图',
  '核心卖点',
...
const LOCAL_ECOMMERCE_PROMPT_CONFIG_BY_KEY = {
  listing_product_set: {
...
export function normalizeLocalDerivedPromptItems(text, limit) {
  const payload = extractJsonPayload(text);
  const source = Array.isArray(payload.prompt_items)
  ...
  return source
    .map((item, index) => {
      ...
      const visibleCopyPoints = normalizeLocalCopyPoints(record.visible_copy_points || record.visibleCopyPoints);
...
export function buildLocalImageExplosionPrompt(node) {
  const data = node?.data || {};
  ...
  const mode = getLocalImageExplosionExtractionMode(data);
...
export function buildLocalEcommerceStrategyPrompt(node) {
  const data = getLocalEffectiveEcommerceData(node?.data || {});
  const capabilityKey = getLocalEcommerceCapabilityKey(data);
  const config = getLocalEcommercePromptConfig(data);
  ...
  const allowedJson = JSON.stringify(buildLocalEcommerceAllowedTypeSummary(data), null, 2);
  const userJson = JSON.stringify(buildLocalEcommerceUserConfigSummary(data), null, 2);
...
export function normalizeLocalEcommerceStrategyResult(text, node) {
  const data = getLocalEffectiveEcommerceData(node?.data || {});
  const payload = extractJsonPayload(text);
  const allowedOptions = getLocalAllowedTypeMap(data);
  const allowedKeys = Array.from(allowedOptions.keys());
  const sourcePlan = Array.isArray(payload.recommended_image_plan) ? payload.recommended_image_plan : [];
  const normalizedPlan = [];
  const aggregatedTypes = new Map();
  let setTotal = 0;

  sourcePlan.forEach((item) => {
    ...
    const normalizedItem = {
      ...
      count,
      goal: stringifyLocalEcommerceField(item.goal || ''),
      visual_idea: stringifyLocalEcommerceField(item.visual_idea || item.visualIdea || ''),
      visible_headline_direction: stringifyLocalEcommerceField(item.visible_headline_direction || item.visibleHeadlineDirection || ''),
      visible_copy_points: normalizeLocalCopyPoints(item.visible_copy_points || item.visibleCopyPoints),
      reason: stringifyLocalEcommerceField(item.reason || ''),
      script_text: stringifyLocalEcommerceField(item.script_text || item.scriptText || ''),
      shot_script: stringifyLocalEcommerceField(item.shot_script || item.shotScript || ''),
    };
    normalizedPlan.push(normalizedItem);
  ...
  if (!setImageTypes.length) {
    ...
    setImageTypes = defaults
      ...
      .map((item) => ({
        key: String(item.key || '').trim() === 'other' ? 'other_ai' : String(item.key || '').trim(),
        enabled: true,
  ...
  setImageTypes = getEcommerceModuleTypeOptions(data).map((option) => ({
    key: option.key,
  ...
  return {
    product_strategy_status: String(payload.product_strategy_status || 'ok').trim() || 'ok',
    capability_key: getLocalEcommerceCapabilityKey(data),
    product_identity: payload.product_identity && typeof payload.product_identity === 'object' && !Array.isArray(payload.product_identity)
      ? payload.product_identity
      : {},
    buyer_decision_factors: Array.isArray(payload.buyer_decision_factors) ? payload.buyer_decision_factors : [],
    product_specific_module_opportunities: Array.isArray(payload.product_specific_module_opportunities)
      ? payload.product_specific_module_opportunities
      : [],
    default_structure_review: payload.default_structure_review && typeof payload.default_structure_review === 'object' && !Array.isArray(payload.default_structure_review)
      ? payload.default_structure_review
      : {},
    capability_strategy: payload.capability_strategy && typeof payload.capability_strategy === 'object' && !Array.isArray(payload.capability_strategy)
      ? payload.capability_strategy
      : {},
    recommended_image_plan: normalizedPlan,
    recommended_config_patch: {
      structureMode: 'custom',
      setImageCount: Math.max(1, setTotal || getLocalEcommerceSetCount(data)),
      setImageTypes,
      userRequirementsAppend: String(payload.user_requirements_append || payload.userRequirementsAppend || '').trim(),
    },
    workflow_injection_brief: String(payload.workflow_injection_brief || '').trim(),
    stage1_guidance: String(payload.stage1_guidance || '').trim(),
    stage2_guidance: String(payload.stage2_guidance || '').trim(),
    avoid_list: Array.isArray(payload.avoid_list) ? payload.avoid_list : [],
    conflict_notes: Array.isArray(payload.conflict_notes) ? payload.conflict_notes : [],
    raw: payload,
  };
}

export function buildLocalEcommerceFallbackStrategyResult(node, reason = '') {
  const data = getLocalEffectiveEcommerceData(node?.data || {});
  const allowedOptions = getLocalAllowedTypeMap(data);
  const defaults = getDefaultEcommerceSetImageTypes(data);
  const recommendedPlan = defaults
    .filter((item) => item?.enabled)
    .map((item) => {
      const key = String(item.key || '').trim() === 'other' ? 'other_ai' : String(item.key || '').trim();
  ...
  return {
    ...
    recommended_config_patch: {
      structureMode: 'custom',
      setImageCount: recommendedPlan.reduce((sum, item) => sum + Math.max(1, Number(item.count || 1) || 1), 0) || getLocalEcommerceSetCount(data),
...
export function buildLocalEcommerceEffectiveConfig(node, strategyResult) {
  const data = node?.data || {};
  const patch = strategyResult?.recommended_config_patch && typeof strategyResult.recommended_config_patch === 'object'
    ? strategyResult.recommended_config_patch
    : {};
  const plannedCount = Math.max(1, Number(patch.setImageCount || getLocalEcommerceSetCount(data)) || getLocalEcommerceSetCount(data));
  return {
    ...
    userRequirements: appendLocalEcommerceText(
...
export function buildLocalEcommerceOverviewPrompt(node) {
  const data = getLocalEffectiveEcommerceData(node?.data || {});
  const config = getLocalEcommercePromptConfig(data);
  ...
  const settings = buildLocalEcommerceCommonSettingLines(data, false);
  const structure = buildLocalEcommerceStructureConfigText(data, 'overview');
  const strategyText = buildLocalOverviewStrategyText(data);
  const textPolicy = buildLocalOverviewTextPolicy(data);
...
export function buildLocalEcommerceSetPrompt(node, overviewAnalysis) {
  const data = getLocalEffectiveEcommerceData(node?.data || {});
  const config = getLocalEcommercePromptConfig(data);
  const count = getLocalEcommerceSetCount(data);
  const textMode = String(data.textMode || 'finished_text').trim();
  const modeKey = textMode === 'clean' ? 'clean' : (textMode === 'rich_text' ? 'rich_text' : 'finished_text');
  const settings = buildLocalEcommerceCommonSettingLines(data, true);
  const structure = buildLocalEcommerceStructureConfigText(data, 'set');
  const strategyText = buildLocalSetStrategyText(data);
  const textPolicy = buildLocalSetTextPolicy(data);
  const focusPolicy = buildLocalFocusPolicy(data);
  const shortVideoRequirements = getLocalEcommerceCapabilityKey(data) === 'short_video_assets'
    ? `短视频脚本要求：
...
`
    : '';
  const shortVideoJsonFields = getLocalEcommerceCapabilityKey(data) === 'short_video_assets'
...
export function getLocalEcommerceSetCount(data = {}) {
  return getEcommerceImageSetCount(data);
}

export function cleanLocalEcommerceVisiblePromptText(prompt, item = {}) {
  ...
  const replacement = sanitizeLocalVisibleHeadline(headline || item.title || item.name || '商品核心信息');
  cleaned = cleaned.replace(
    /\s*注意：image_category、title、模块名和系统归类标签仅用于内部管理，不能作为图片可见主标题；图片中的可见文字必须改写为面向买家的自然商品文案。(?:\s*建议可见主标题方向：[^。]*。)?(?:\s*建议短文案方向：[^。]*。)?/gu,
    ''
  );
  LOCAL_ECOMMERCE_INTERNAL_LABEL_TERMS.forEach((term) => {
    const escaped = escapeLocalRegExp(term);
...
function getLocalImageExplosionExtractionMode(data = {}) {
  const mode = String(data.extractionMode || (data.explosionMode === 'clean' ? 'clean_material' : data.explosionMode) || 'clean_material')
    .trim()
    .toLowerCase();
  return mode === 'original' ? 'original' : 'clean_material';
}

function getLocalEffectiveEcommerceData(data = {}) {
...
function getLocalEcommerceCapabilityKey(data = {}) {
  return getEcommerceCapability(data);
}

function getLocalEcommercePromptConfig(data = {}) {
  const capabilityKey = getLocalEcommerceCapabilityKey(data);
  const base = getEcommerceCapabilityConfig(data) || {};
  const promptConfig = LOCAL_ECOMMERCE_PROMPT_CONFIG_BY_KEY[capabilityKey] || LOCAL_ECOMMERCE_PROMPT_CONFIG_BY_KEY.listing_product_set;
  return {
    label: String(base.label || promptConfig.label || '电商图片'),
    shortLabel: String(base.shortLabel || promptConfig.shortLabel || base.label || '电商图'),
    description: String(base.description || promptConfig.description || '生成电商图片素材'),
    ...promptConfig,
  };
}

function getLocalAllowedTypeMap(data = {}) {
  return new Map(getEcommerceModuleTypeOptions(data).map((option) => [String(option.key || '').trim(), option]));
}

function buildLocalEcommerceAllowedTypeSummary(data = {}) {
  const typeMap = getLocalAllowedTypeMap(data);
  const types = Array.from(typeMap.values()).map((option) => ({
...
function buildLocalEcommerceUserConfigSummary(data = {}) {
...
function buildLocalEcommerceCommonSettingLines(data = {}, includeFocusSingleProduct = true) {
  const capabilityKey = getLocalEcommerceCapabilityKey(data);
  const lines = [];
  const labelMap = {
    listing_product_set: {
  ...
  buildLocalEcommerceCapabilitySettingLines(data, capabilityKey).forEach((line) => lines.push(line));
...
function buildLocalEcommerceCapabilitySettingLines(data = {}, capabilityKey) {
...
function buildLocalEcommerceStructureConfigText(data = {}, stage = 'overview') {
  const count = getLocalEcommerceSetCount(data);
  const config = getLocalEcommercePromptConfig(data);
  ...
  if (stage === 'set') {
    return `${baseText} 第二阶段输出 JSON 时要逐条展开这些内部类型；同一类型重复出现时，每张图必须有不同职责和构图。生成 prompt 时必须把内部类型改写成买家能理解的商品画面主题，不要把后台归类名称直接写成图片可见标题或生图主题。AI补充图不能直接写成“其他图”，必须根据商品特征解析成具体、可执行、面向买家的图片方向。`;
  }
  return `${baseText} 第一阶段只需要把这些内部类型方向体现在单张${config.overviewName}的规划里，不要输出多张图片；展示单元的可见文案也必须使用商品语言，不要直接展示内部归类名称。AI补充图可以作为启发模型扩展商品专属展示方向。`;
}

function buildLocalOverviewStrategyText(data = {}) {
  const parts = [];
...
function buildLocalSetStrategyText(data = {}) {
  const parts = [];
  const brief = String(data.ecommerceStrategyBrief || '').trim();
  ...
  const recommendedPlanText = buildLocalEcommerceRecommendedPlanText(recommendedPlan);
...
function buildLocalOverviewTextPolicy(data = {}) {
  const textMode = String(data.textMode || 'finished_text').trim();
  if (textMode === 'clean') {
...
function buildLocalSetTextPolicy(data = {}) {
  const textMode = String(data.textMode || 'finished_text').trim();
  if (textMode === 'clean') {
...
function buildLocalFocusPolicy(data = {}) {
  if (data.focusSingleProduct) {
    return '聚焦单品已开启：每条图像提示词都要优先描述单个商品本体在一个明确画面中的聚焦展示；允许局部放大、小插图、多角度或同一商品状态示意，但不要让画面主体变成多件并列陈列。';
  }
  return '聚焦单品未开启：保留当前多视角、多形态、丰富展示策略；可以在合理范围内使用双形态并列、多状态小图、局部放大和场景拼接，让电商图片信息更完整。';
}

function appendLocalEcommerceText(base, append, label = '') {
  const baseText = String(base || '').trim();
  const appendText = String(append || '').trim();
  if (!appendText) {
    return baseText;
  }
  if (!baseText) {
  ...
  return `${baseText}\n${label ? `${label}：` : ''}${appendText}`;
}

function buildLocalEcommerceRecommendedPlanText(plan) {
  const normalized = Array.isArray(plan)
    ? plan
      ...
      .map((item, index) => ({
        ...
        count: Math.max(1, Number(item.count || 1) || 1),
        goal: stringifyLocalEcommerceField(item.goal || ''),
        visual_idea: stringifyLocalEcommerceField(item.visual_idea || ''),
        visible_headline_direction: stringifyLocalEcommerceField(item.visible_headline_direction || ''),
        visible_copy_points: normalizeLocalCopyPoints(item.visible_copy_points),
        reason: stringifyLocalEcommerceField(item.reason || ''),
        script_text: stringifyLocalEcommerceField(item.script_text || ''),
        shot_script: stringifyLocalEcommerceField(item.shot_script || ''),
      }))
    : [];
  return normalized.length ? JSON.stringify(normalized) : '';
}

function stringifyLocalEcommerceField(value) {
...
function normalizeLocalCopyPoints(value) {
...
function sanitizeLocalVisibleHeadline(value) {
...
function escapeLocalRegExp(value) {
  return String(value || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

function getIncomingNodes(nodeId, nodes, edges) {
...
function getNodeResultImages(node, resultsByNode) {
  const runtimeItems = resultsByNode.get(node.id) || [];
  const storedItems = Array.isArray(node.data?.resultItems) ? node.data.resultItems : [];
  const urls = runtimeItems
    .concat(storedItems)
    .map((item) => String(item?.downloadUrl || item?.referenceUrl || item?.imageUrl || item?.download_url || item?.reference_url || item?.image_url || '').trim())
    .filter(Boolean);
  const direct = String(node.data?.annotatedImageUrl || node.data?.referenceUrl || node.data?.outputUrl || node.data?.imageUrl || '').trim();
  if (direct) {
    urls.push(direct);
  }
...
function buildAuthHeaders(input) {
  const apiKey = String(input?.imagesApiKey || '').trim();
  const authMode = String(input?.preferredAuthMode || 'bearer').trim().toLowerCase();
  const headers = {
    Accept: 'application/json',
  };
  if (!apiKey) {
...
async function imageSourceToFile(imageUrl, index, signal) {
  const value = String(imageUrl || '').trim();
  if (!value) {
    throw new Error('参考图为空，无法提交图生图请求。');
  }
  let response;
  try {
    response = await window.fetch(value, { cache: 'no-store', signal });
  } catch (error) {
    if (error?.name === 'AbortError') {
      throw error;
    }
    throw new Error('本地模式无法读取参考图。请尽量使用当前浏览器上传的本地图片或本地模式刚生成的图片。');
  }
  if (!response.ok) {
    throw new Error(`本地模式读取参考图失败：HTTP ${response.status}`);
  }
  const blob = await response.blob();
  if ((blob.size || 0) > CANVAS_REFERENCE_IMAGE_MAX_BYTES) {
    throw new Error('参考图超过 12MB，请压缩后再试。');
  }
  const extension = detectImageExtension(blob.type, value);
  return new File([blob], `reference-${index + 1}.${extension}`, {
    type: blob.type || OUTPUT_FORMAT_MIME.png,
    lastModified: Date.now(),
  });
}

function detectImageExtension(mimeType, source) {
...
function extractJsonPayload(text) {
  ...
  if (direct && typeof direct === 'object') {
    return Array.isArray(direct) ? { prompt_items: direct } : direct;
  }
  ...
  if (objectStart >= 0 && objectEnd > objectStart) {
    ...
    if (parsed && typeof parsed === 'object') {
      return Array.isArray(parsed) ? { prompt_items: parsed } : parsed;
    }
  }
...
async function tryLocalizeRemoteImageUrl(imageUrl, signal) {
  const value = String(imageUrl || '').trim();
  if (!/^https?:\/\//i.test(value)) {
    return '';
  }
  try {
    const response = await window.fetch(value, { cache: 'no-store', signal });
    if (!response.ok) {
      return '';
    }
    const blob = await response.blob();
...
function createLocalTaskId() {
  return `localimg_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
}
