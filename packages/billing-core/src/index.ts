...
/**
 * CNY is stored as an integer number of one hundred-thousandth yuan.
 * Keeping all ledger math in integers avoids floating point rounding drift.
 */
export const CNY_MINOR_UNIT_SCALE = 100_000;
export const CNY_MINOR_UNIT_FRACTION_DIGITS = 5;

function normalizeMoneyInput(value: number | string | null | undefined) {
...
/** Converts yuan to the canonical integer minor unit, rounded to 0.00001 yuan. */
export function yuanToMinorUnits(value: number | string | null | undefined) {
  const parsed = normalizeMoneyInput(value);
  if (!parsed) {
    return 0;
  }
  ...
  const padded = `${parsed.fraction}000000`;
  let fraction = Number(padded.slice(0, CNY_MINOR_UNIT_FRACTION_DIGITS));
  if (Number(padded[CNY_MINOR_UNIT_FRACTION_DIGITS] || '0') >= 5) {
    fraction += 1;
  }
  ...
  return parsed.negative ? -absolute : absolute;
}

/** Converts canonical integer minor units to a yuan number for API payloads. */
export function minorUnitsToYuan(value: number | null | undefined) {
  const normalized = Math.trunc(Number(value || 0));
  return normalized / CNY_MINOR_UNIT_SCALE;
}

/** Formats canonical integer minor units without hiding meaningful precision. */
export function formatCnyMinorUnits(value: number | null | undefined, options?: {
  ...
  const minimumFractionDigits = Math.max(0, Math.min(CNY_MINOR_UNIT_FRACTION_DIGITS, Math.floor(Number(options?.minimumFractionDigits || 0))));
