...
const controlPlaneSchema = z.object({
  ...
  publicApi: z.object({
    ...
    asyncQueuePollMs: z.number().int().min(250).max(60_000),
    asyncQueueWaitMs: z.number().int().min(5_000).max(30 * 60_000),
    maxInputImageMb: z.number().min(1).max(12),
    maxInputImageCount: z.number().int().min(1).max(6),
    maxInputImageTotalMb: z.number().min(1).max(30),
    ...
    return {
      ...
      plans: [smartPlan, preferredPlan].map((plan) => ({
        ...
        candidates: plan.candidates.map((candidate, index) => ({
          ...
          healthEvidenceAgeMs: Math.round(candidate.healthEvidenceAgeMs),
          healthEvidenceFreshness: Number(candidate.healthEvidenceFreshness.toFixed(4)),
          successLatencyFreshness: Number(candidate.successLatencyFreshness.toFixed(4)),
